...
const Form = ({formControls, formData, setFormData, onSubmit, buttonText, isBtnDisabled}) => {
    ...
    function randomInputByComponentType(getControlItem){
        let element = null
  ...
  return (
    <form onSubmit={onSubmit}>
        <div className='flex flex-col gap-3'>
            {
                formControls.map(controlItem=> <div key={controlItem.name} className='grid w-full gap-5'>
                    <Label className="mb-1">{controlItem.label}</Label>
                    {
                        randomInputByComponentType(controlItem)
                    }
                </div>)
            }
        </div>
